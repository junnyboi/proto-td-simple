...
func _run() -> void:
	...
	_battle.set("_operator_anim_seconds", 0.875)
	...
	var recruit_count := 1 if portrait else 2
	_expected_count = unit_count + recruit_count
	var cells := _representative_cells(_expected_count)
	if cells.size() != _expected_count:
		push_error("advanced live quality harness: expected %d visible cells, got %d" % [_expected_count, cells.size()])
		quit(4)
		return
	...
	if portrait:
		...
		for gender_index: int in 2:
			...
			for facing_index: int in FACINGS.size():
				...
				BattleTicketRuntimeScript.copy_legacy_unit(operator, unit)
				model.units.append(unit)
	# Two excluded recruit identities provide a live scale/grounding benchmark.
	var recruit_def := load("res://data/operators/recruit.tres") as OperatorDef
	for recruit_index: int in recruit_count:
		var recruit := UnitStateScript.new()
		recruit.id = 2500 + recruit_index
		var recruit_gender := _portrait_gender if portrait else ("female" if recruit_index == 0 else "male")
		recruit.hero_id = _hero_id_for_gender(recruit_gender, recruit.id)
		recruit.cell = cells[unit_count + recruit_index]
		recruit.facing = UnitStateScript.Facing.RIGHT
		BattleTicketRuntimeScript.copy_legacy_unit(recruit_def, recruit)
		model.units.append(recruit)
	_suppress_overlays()
	await _capture_state("idle", -1, 0)
	await _capture_state("attack", 0, 7)
	await _capture_state("attack", 0, 16)
	await _capture_state("attack", 0, 28)
	await _capture_state("idle", 0, 34)
	...
	for _frame: int in 4:
		await process_frame
	print("OPERATOR_V3_LIVE_QUALITY_OK class=%s prefix=%s" % [_class_id, _output_prefix])
	quit(0)
...
func _capture_state(state: String, last_attack_tick: int, model_tick: int) -> void:
	...
	for unit: UnitState in model.units:
		...
		var offsets := {
			UnitStateScript.Facing.RIGHT: Vector2i.RIGHT,
			UnitStateScript.Facing.DOWN: Vector2i.DOWN,
			UnitStateScript.Facing.LEFT: Vector2i.LEFT,
			UnitStateScript.Facing.UP: Vector2i.UP,
		}
		unit.last_attack_cell = unit.cell + (offsets[unit.facing] as Vector2i)
...
func _validate_projection(state: String) -> void:
	...
	for unit: UnitState in (_battle.get("model") as BattleModel).units:
		...
		var is_recruit := unit.op_id == &"recruit"
		var expected_template := StringName("%s_%s" % ["recruit" if is_recruit else _class_id, gender])
		...
		var projected_contact := body.position + body.size * animation.pivot
		var target_contact := Vector2(0, OperatorAnimatorScript.ground_offset(animation))
		if not projected_contact.is_equal_approx(target_contact):
			push_error("unit %d foot anchor mismatch %s vs %s" % [unit.id, projected_contact, target_contact])
			quit(14)
		if not is_recruit and animation.display_height_px != 58:
			push_error("unit %d does not use recruit display scale" % unit.id)
			quit(15)
		var selected := OperatorAnimatorScript.selection(
			unit,
			(_battle.get("model") as BattleModel).tick,
			float(_battle.get("_operator_anim_seconds")),
			animation,
		)
...
func _representative_cells(count: int) -> Array[Vector2i]:
	...
	for y: int in size.y:
		for x: int in size.x:
			...
			if stage.tile_at(cell) in [StageDef.Tile.GROUND, StageDef.Tile.ELEVATED]:
...
func _suppress_overlays() -> void:
	...
	for line: Line2D in (_battle.get("_tracer_lines") as Dictionary).values():
		line.visible = false
