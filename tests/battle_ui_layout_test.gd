...
func _run() -> void:
	...
	_check(tutorial_card != null and tutorial_card.get_theme_stylebox(&"panel") is StyleBoxFlat, "tutorial card still uses a stylized frame")
	if tutorial_card != null:
		var tutorial_rect := tutorial_card.get_global_rect()
		var tutorial_style := tutorial_card.get_theme_stylebox(&"panel") as StyleBoxFlat
